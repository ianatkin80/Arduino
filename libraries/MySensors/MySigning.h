 ...
 *
 * Before we begin with the details, I just want to emphasize that signing is completely optional and not enabled by default, it will use a
 * default backend that does not require signing and does not enforce signature checks. If you really do want the additional "cost" in
 * program space related to signing, you can enable @ref MY_SIGNING_FEATURE in MyConfig.h.
 *
 * <b>Firstly</b>, you need to make sure @ref MY_SIGNING_FEATURE is enabled in MyConfig.h. You then select which backend to use by passing the
 * appropriate handle when constructing the MySensor object. The handle is passed as the third argument (example here uses the real ATSHA
 ...
 *
 * <b>Secondly</b>, you need to verify the configuration for the backend. Currently, only MySigningAtsha204 and MySigningAtsha204Soft backends
 * have a specific configuration.<br>
 * For MySigningAtsha204 it is the pin the device is connected to. In MyConfig.h there are defaults for sensors and gateways which you might
 * need to adjust to match your personal build. The setting is defined using @ref MY_ATSHA204_PIN and the default is to use pin A3.<br>
 * For MySigningAtsha204Soft, an unconnected analog pin is required to set a random seed for the pseudo-random generator. It is important that
 * the pin is floating, or the output of the pseudo-random generator will be predictable, and thus compromise the signatures. The setting is
 * defined using @ref MY_RANDOMSEED_PIN and the default is to use pin A7.
 *
 * <b>Thirdly</b>, if you use the MySigningAtsha204Soft backend, you need to create/set a HMAC key to use. This key is 32 bytes wide and should
 ...
 * key is stored in our sketch and is passed when constructing the MySigningAtsha204Soft object.
 *
 * An example of a node that require signatures is available in @ref SecureActuator.ino.
 *
 * If you use the “real” ATSHA204A, before any signing operations can be done, the device needs to be <i>personalized</i>. This can be a
 * daunting process as it involves irreversibly writing configurations to the device, which cannot be undone. I have however tried to simplify
 * the process as much as possibly by creating a helper-sketch specifically for this purpose in
 ...
 * personalizer for this purpose.
 * 
 * Signing in the MySensors network is driven from the receiving nodes. That means, if a node require signing it will inform the gateway of this.
 * To instruct a node to require signing by the gateway, provide a suitable backend to the library constructor. Both MySigningAtsha204 and
 * MySigningAtsha204Soft backends will by-default require signing when used.<br>
 * The default constructors for these backends can be overridden to disable signing requirements if the node does not require signed messages but
 * still need the ability to verify messages (like a gateway).
 *
 ...
 *
 * None of this activity is “visible” to you (as the sensor sketch implementor). All you need to do is to set your preferences in MyConfig.h,
 * depending on chosen backend, do personalization or key configurations and set the @c requestSignatures parameter to @c true. That is enough to
 * enable protection from both Eve and Mallory in your network (although because of the lack of encryption, Eve can eavesdrop, but not do
 ...
 *
 * This is achieved by 'salting' the signature with some node-unique information known to the receiver. In the case of ATSHA204A this is the unique
 * serial number programmed into the circuit. This unique number is never transmitted over the air in clear text, so Eve will not be able to figure
 * out a "trusted" serial by snooping on the traffic.<br>
 ...
 * provide and use whitelists, as well as transmitting to a node with a whitelist, you need to enable @ref MY_SECURE_NODE_WHITELISTING in
 * MyConfig.h.<br>
 ...
 * constructor argument when you enable whitelisting as illustrated in this example:
 * @code{.cpp}
 * #include <MySigningAtsha204Soft.h>
 * MyTransportNRF24 radio;  // NRFRF24L01 radio driver
 * MyHwATMega328 hw; // Select AtMega328 hardware profile
 ...
 * @endcode
 *
 * For a node that should transmit whitelisted messages but not receive whitelisted messages, you can simply skip the whitelist arguments (@c 1 and
 * @c node_whitelist above). For the "soft" backend, you can set these to @c 0 and @c NULL since you then need to provide the @c soft_serial buffer.
 *
 ...
 * in question. You only need to have entries for the nodes that in turn have enabled @ref MY_SECURE_NODE_WHITELISTING. Nodes that does not have
 ...
 *
 * As for the ATSHA204SOFT backend, it turns out that the ATSHA does not do “vanilla” HMAC processing. Fortunately, Atmel has documented exactly how
 ...
 * In the MySensors protocol, the following new internal messagetypes has been added for handling signature requirements and nonce requests:<br>
 ...
 *
 * It is very important to emphasize that with the current implementation of message signing, <b>OTA firmware updates are transmitted unsigned</b>.
 * In other words, it is technically possible for Mallory to reset a node by cutting power or some other attack, spoof a gateway and push his/her
 * own custom firmware to the node even if the old sketch was requiring signing. The current architecture of the OTA solution prevents signing
 * support from being implemented in the bootloader due to size constraints.<br>
 * It is still possible to use OTA bootloader with a signing gateway and node, but it is important to understand this <b>potentially provides an
 * attack vector for compromising the security of the node</b>.
 * Also, due to the limiting factor our our Arduino nodes, the use of diversified keys is not implemented. That mean that all nodes in your
 ...
 * this, and that is hopefully covered in the "Typical usecases" chapter below.<br>
...
/**
 * @class MySigning MySigning.h
 * @brief Interface class to the signing backend(s)
 *
 * This class defines the API to manage the signing backend(s).
 */
class MySigning
{ 
public:
	/// @brief Construct the signing backend.
        ///
	/// Different parameters are needed depending on signing backend (e.g. pins connected etc.).
	/// Keeping these parameters as \#define's in MyConfig.h to streamline the driver interface.
	...
	MySigning(bool requestSigs=false);

	/// @brief Returns the current signature request setting
	///
	/// If this is @c false, node is configured to not require signed messages and will accept
	/// unsigned messages. The setting is controlled by the constructor @ref MySigning().
	/// \n@b Usage: This function should be used when presenting a node in the network with the
	/// @ref I_REQUEST_SIGNING message and it should be used when receiving a message to determine if it is to be rejected
	/// or not.
	/// @returns The current signature request setting
	...
	/// \n@b Usage: This function should be called on regular intervals like typically within some process loop.
	...
	/// message. @param msg The message to get the nonce from.
	...
	///
	/// Nonce used for signature calculation is the one generated previously using @ref getNonce().
	...
	/// \n@b Usage: This function is typically called as action when receiving a @ref I_GET_NONCE_RESPONSE
	/// message and after @ref putNonce() has successfully been executed.
	...
	///
	/// Nonce used for verification is the one previously set using @ref putNonce().
	/// Nonce will be cleared when this function is called to prevent re-use of nonce.
	/// Returns false if subsystem is busy processing an ongoing signing operation.
	/// Returns false if signing identifier found in message is not supported by the used backend.
	...
	/// and @ref requestSignatures() indicate signatures are required.
